...
describe("api-commit-and-push", async () => {
  ...
  before(async () => {
    directory = await fs.promises.mkdtemp(
      path.join(os.tmpdir(), "api-commit-and-push-"),
    );
    ...
    await fs.promises.writeFile(
      path.join(directory, "somedir", "somefile.rb"),
      "test file 1",
    );
    await fs.promises.writeFile(
      path.join(directory, removedFile),
      "test file 2",
    );
    ...
    await fs.promises.mkdir(path.join(directory, path.dirname(addedFile)), {
      recursive: true,
    });
    await fs.promises.writeFile(path.join(directory, addedFile), "test file 3");
    await exec.exec("git", ["-C", directory, "add", "-A"]);
    baseCommitSha = (
      await exec.getExecOutput("git", ["-C", directory, "rev-parse", "HEAD"])
    ).stdout.trim();
    baseTreeSha = (
      await exec.getExecOutput("git", ["-C", directory, "rev-parse", "HEAD:"])
    ).stdout.trim();
      ...
      .intercept({
        ...
        body: (body) =>
          util.isDeepStrictEqual(JSON.parse(body), {
            content: fs.readFileSync(path.join(directory, addedFile), {
              encoding: "base64",
            }),
            encoding: "base64",
          }),
