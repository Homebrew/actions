...
async function main() {
  try {
    ...
    const autosquash_label = core.getInput("autosquash_label", {
      required: true,
    });
    ...
    // Parse event
    const json = fs.readFileSync(process.env.GITHUB_EVENT_PATH, {
      encoding: "utf-8",
    });
    ...
    for (const commit of commits.data) {
      ...
      const commit_subject = commit_info.data.commit.message
        .split("\n")
        .shift();
      ...
      if (file.filename.startsWith("Formula/")) {
        ...
        if (
          files_touched.includes(file.filename) ||
          !commit_subject.startsWith(formula)
        ) {
          autosquash = true;
          commit_state = "failure";
          message =
            "Please follow the commit style guidelines, or this pull request will be replaced.";
        }
        files_touched.push(file.filename);
      } else if (file.filename.startsWith("Casks/")) {
        message = "Commit modifies cask.";
        target_url =
          "https://github.com/Homebrew/homebrew-cask/blob/HEAD/CONTRIBUTING.md#style-guide";
    ...
    if (is_success && existingLabels.includes(failure_label)) {
      ...
      !is_success &&
      !existingLabels.includes(failure_label) &&
      !existingLabels.includes(ignore_label)
    ) {
    ...
    if (!autosquash && existingLabels.includes(autosquash_label)) {
      ...
      autosquash &&
      !existingLabels.includes(autosquash_label) &&
      !existingLabels.includes(ignore_label)
    ) {
    ...
    if (
      existingLabels.length == updatedLabels.length &&
      existingLabels.every((label, i) => label == updatedLabels[i])
    ) {
      return;
    }
